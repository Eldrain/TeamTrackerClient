...
public class World implements MouseListener, ActionListener {
    ...
    Graphics gr;
    InfObj info;
    String serviceName = "rmi://localhost/Info";

    public World(Graphics gr) throws RemoteException, MalformedURLException {
        win = false;
        player = true;
        step = 1;
        actions = 3;
        ...
        /*try {
            ...
            Info stub = (Info) UnicastRemoteObject.exportObject(info, 1098);

            // Bind the remote object's stub in the registry
            Registry registry = LocateRegistry.createRegistry(1098);
            registry.rebind("Info", stub);
        ...
        info = new InfObj();
        //Registry registry = LocateRegistry.createRegistry(1098);
        //Naming.rebind(serviceName, info);
    }

    public void clickHandler(int x, int y) throws RemoteException {
        if(!inBox(x, y) || win)
            return;
